    ...
    try:
        ...
        if typeOfLocation == 0:      
            ...
            if not key and not operator:
                ...
                cursor.execute("SELECT startedAt, uid, latency, downlink, uplink, postalcode, networkoperator FROM otaniemitesti3 WHERE startedAt BETWEEN adddate(%s,%s) AND %s AND key = %s",(time,lengthOfTime,time,key,))
                ...
                cursor.execute("SELECT startedAt, uid, latency, downlink, uplink, postalcode, networkoperator FROM otaniemitesti3 WHERE startedAt BETWEEN adddate(%s,%s) AND %s AND key = %s AND networkoperator = %s",(time,lengthOfTime,time,key,operator))
        ...
        if typeOfLocation == 1:
            ...
            if not key and not operator:
                ...
                cursor.execute("SELECT startedAt, uid, latency, downlink, uplink, postalcode, networkoperator FROM otaniemitesti3 WHERE startedAt BETWEEN adddate(%s,%s) AND %s AND key = %s AND postalcode LIKE %s",(time,lengthOfTime,time,key,location,))
            elif not key and operator:
                ...
                cursor.execute("SELECT startedAt, uid, latency, downlink, uplink, postalcode, networkoperator FROM otaniemitesti3 WHERE startedAt BETWEEN adddate(%s,%s) AND %s AND key = %s AND networkoperator = %s AND postalcode LIKE %s",(time,lengthOfTime,time,key,operator,location))
        ...
        if typeOfLocation == 2:
            ...
            if not key and not operator:
                ...
                cursor.execute("SELECT startedAt, uid, latency, downlink, uplink, postalcode, networkoperator FROM otaniemitesti3 WHERE startedAt BETWEEN adddate(%s,%s) AND %s AND key = %s AND county = %s",(time,lengthOfTime,time,key,location,))
            elif not key and operator:
                ...
                cursor.execute("SELECT startedAt, uid, latency, downlink, uplink, postalcode, networkoperator FROM otaniemitesti3 WHERE startedAt BETWEEN adddate(%s,%s) AND %s AND key = %s AND networkoperator = %s AND county = %s",(time,lengthOfTime,time,key,operator,location))
...
def getAveragesSortOperators(time, key, weekly, fir, location, typeOfLocation, timeWindow, resolution, sortOperators):   
